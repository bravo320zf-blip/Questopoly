...
const ABILITY_LIBRARY = {
    // ================= FIGHTER SKILLS =================
power_strike: { 
        name: "Power Strike", desc: "Gain +5 STR for this turn only.", 
        fn: (p) => { 
    ...
    intimidate: { 
        name: "Intimidate", 
        desc: "Force player to pay 50G. Recharges on Enemy Capture.", // UPDATED
        fn: (p) => { 
...
second_wind: { 
        name: "Second Wind", desc: "Gain 150 Gold immediately.", 
        fn: (p) => { 
    ...
    shield_bash: { 
        name: "Shield Bash", desc: "Stun a random enemy (Skip Next Turn).", 
        fn: (p) => { 
            const targets = players.filter(t=>t.id!==p.id&&!t.isDead); 
            if(targets.length){
                const t=targets[Math.floor(Math.random()*targets.length)]; 
    ...
    rally: { 
        name: "Rally", desc: "Gain +2 to all Stats for this turn.", 
        fn: (p) => { 
...
fireball: { 
        name: "Fireball", 
        desc: "Pay 100G: Burn property. Guards die first. Splash 100G Dmg.", 
        fn: (p) => { 
    ...
    transmute_gold: { 
        name: "Transmute", desc: "Pay 50G: Destroy item -> Draw Treasure.", 
        fn: (p) => { 
            if(p.inventory.length>0 && p.gold>=50){
                VFX.trigger('pillar', p.mesh.position, null, 0xffd700);
                p.gold-=50; p.inventory.splice(0,1); 
                const c=DECK_TREASURE[Math.floor(Math.random()*DECK_TREASURE.length)]; 
                p.inventory.push(createItemInstance(c)); 
                addLog("Transmuted item!", "log-gold"); return true;
            } return false; 
        }, 
        img: "https://static.wixstatic.com/media/b16479_765369c1c1bd4a8abb1b752a2245f56c~mv2.jpg" 
    },
    blink: { 
        name: "Blink", desc: "Move 1-6 spaces immediately.", 
        fn: (p) => { 
    ...
    time_warp: { 
        name: "Time Warp", desc: "Roll 1d6. On 5-6, take another turn immediately.", 
        fn: (p) => { 
...
polymorph: { 
        name: "Polymorph", 
        desc: "Turn enemy into a Sheep (1 Turn).", 
        fn: (p) => { 
    ...
    arcane_eye: { 
        name: "Arcane Eye", desc: "Teleport to the nearest Chest.", 
        fn: (p) => { 
...
pickpocket: { 
        name: "Pickpocket", 
        desc: "Steal 10G/prop from the land leader. Recharges every 10 moves.", 
        fn: (p) => { 
    ...
    sprint_act: { 
        name: "Sprint", 
        desc: "Move 3 spaces. Recharges every 10 moves.", 
        fn: (p) => { 
    ...
    smoke_bomb_act: { 
        name: "Smoke Bomb", desc: "Teleport to random space (Escape).", 
        fn: (p) => { 
...
sabotage: { 
        name: "Sabotage", 
        desc: "Choose a Tavern to downgrade (Level 2 -> 1).", 
        fn: (p) => { 
...
shadow_strike: { 
    name: "Shadow Strike", desc: "Teleport to target, Steal 100G. -1 Stats.", 
    fn: (p) => { 
        ...
        showPlayerPicker("Shadow Strike Target", enemies, (target) => {
            ...
            // REMOVED: p.classSkillDepleted = true;  <-- THIS WAS THE BUG
            updateHUD();
        });
        return true;
    }, 
    ...
    // ================= CLERIC SKILLS =================
    heal_spell: { name: "Heal", desc: "Clear negative status (Jail/Stun).", fn: (p) => { VFX.trigger('spiral_up', p.mesh.position, null, 0x00ff00); if(p.isSkipping){p.isSkipping=false; addLog("Healed status!", "log-success"); return true;} addLog("No status to heal.", "log-fail"); return false; }, img: "https://static.wixstatic.com/media/b16479_6fbb98ea84a9479b98dd0441fd7152b6~mv2.jpg" },
    smite: { 
        name: "Smite", 
        desc: "Instant Capture (or Upgrade Camp to Tavern) on current tile.", 
        fn: (p) => { 
            const t = tiles[p.pos]; 
    ...
    bless: { 
        name: "Bless", desc: "Gain +5 INT for this turn.", 
        fn: (p) => { 
...
exorcism: { 
    name: "Exorcism", desc: "Destroy random enemy equipment (or -20G).", 
    fn: (p) => { 
        const enemies = players.filter(x => x.id !== p.id && !x.isDead && !x.isEventEntity);
        if (enemies.length === 0) return false;
...
shadow_step: { 
    ...
    greedy: { 
        name: "Greed", 
        desc: "Start with +200G and a random Treasure Card.", // UPDATED TEXT
        type:'passive', 
        // Note: Logic is now in spawnPlayer to prevent infinite gold on recalc
    ...
    transmute: { 
        ...
        fn: (p) => { 
            ...
            addLog("Inventory empty!", "log-fail");
            return false; 
        }, 
...
// Call this from triggerTileEvent() when landing on a Dungeon Entrance
function offerDungeonEntry(p, tileName) {
...
function resolveDungeonStep(p) {
    const config = DUNGEON_DATA[activeDungeon.type];
    const diff = getDungeonDifficulty();
    
    // --- NEW: BOSS AMBUSH CHECK ---
    const bossRoll = Math.floor(Math.random() * 6) + 1;
    
    if (bossRoll === 6) {
        addLog(`${p.name} rolled a 6...`, "log-warning");
        setTimeout(() => {
            addLog("THE BOSS AMBUSHES YOU!", "log-fail");
            AUDIO.playSound('sfx_transition');
            // Trigger Boss with 'isAmbush' flag true
    ...
    const tn = diff.tnBase + 1;
    
    const m = document.getElementById('card-modal');
    document.getElementById('enc-title').innerText = "Dungeon: " + enc.name;
    document.getElementById('enc-desc').innerText = `Safe (Roll 1-5). ${diff.tier} Threat! Roll ${enc.stat.toUpperCase()} vs TN ${tn}.`;
    document.getElementById('enc-header').className = 'card-header skirmish';
    ...
    let b = document.createElement('div');
    b.className = 'choice-btn';
    b.innerText = `Fight (${enc.stat.toUpperCase()})`;
    b.onclick = () => {
        let pool = p.stats[enc.stat] || 1;
        let wins = 0;
        ...
        if (wins >= diff.req) {
            addLog(`${p.name} cleared the room! Found loot.`, "log-success");
            ...
            addLog(`${p.name} was defeated and fled to the Inn!`, "log-fail");
            AUDIO.playSound('sfx_fail');
            p.inDungeon = false;
            p.dungeonProgress = 0;
            p.pos = 0;
            animateTeleport(p, tiles[0], () => {
                m.classList.remove('active');
                endStep();
            });
        }
...
function triggerDungeonBoss(p, isAmbush = false) {
    // Safety Check: Ensure player knows which dungeon they are in
    if (!p.dungeonType || !activeDungeons[p.dungeonType]) {
        console.error("Boss Trigger Error: Dungeon data missing for " + p.name);
        return;
    }

    const myDungeon = activeDungeons[p.dungeonType];
    const config = DUNGEON_DATA[p.dungeonType];
    
    const attempts = p.bossAttempts || 3;
    const diff = getDungeonDifficulty();
    const tn = diff.tnBase + 1;
    const req = diff.req + 1; // Bosses are harder than normal rooms

    // 1. Setup Boss Visuals
    const boss = myDungeon.bossMesh;
    
    // Reset transparency from the fade-in cinematic
    boss.traverse((child) => { 
        if(child.isMesh) {
    ...
    boss.visible = true;
    boss.scale.set(1.5, 1.5, 1.5); // Ensure normal size

    // Position Boss
    if (isAmbush) {
        // Teleport boss to player's current tile
        boss.position.copy(p.mesh.position);
        boss.position.y = 0; 
        boss.position.z -= 1.5; // Slight offset so they face each other
        boss.lookAt(p.mesh.position);
    } else {
        // Standard End Boss Position (Last tile of THIS dungeon)
        const endTile = myDungeon.tiles[myDungeon.tiles.length - 1];
        boss.position.copy(endTile.position);
        boss.position.y = 0;
        
        // Orient boss towards the path
        if (myDungeon.tiles.length > 1) {
            const prevTile = myDungeon.tiles[myDungeon.tiles.length - 2];
            boss.lookAt(prevTile.position);
        }
    }

    VFX.trigger('smoke', boss.position);
    AUDIO.playSound('sfx_transition');

    // 2. UI Modal Setup
    const m = document.getElementById('card-modal');
    const title = isAmbush ? `AMBUSH: ${config.bossName}` : `BOSS: ${config.bossName}`;
    document.getElementById('enc-title').innerText = title;
    document.getElementById('enc-desc').innerText = `Attempts Left: ${attempts}. Roll Highest Stat vs TN ${tn}. (${req} successes needed)`;
    document.getElementById('enc-header').className = 'card-header skirmish';
    ...
    // Determine Best Stat
    const highestStat = (p.stats.str >= p.stats.dex && p.stats.str >= p.stats.int) ? 'str' : (p.stats.dex >= p.stats.int ? 'dex' : 'int');

    let b = document.createElement('div');
    b.className = 'choice-btn';
    b.style.borderColor = '#ef4444';
    b.innerHTML = `<span>ATTACK BOSS</span> <span style="font-size:0.8rem">Use ${highestStat.toUpperCase()}</span>`;
    
    b.onclick = () => {
        // Roll Logic
        let pool = p.stats[highestStat] || 1;
        let wins = 0;
        let rolls = [];
        ...
        rolls.forEach(r => {
            ...
            die.className = 'mini-die ' + (r >= tn ? 'win' : 'lose');
            die.innerText = r;
            resDiv.appendChild(die);
        });
        ...
        setTimeout(() => {
            if (wins >= req) {
                // --- VICTORY ---
                globalLog(`${p.name} SLAIN THE ${config.bossName}!`, "log-epic");
                AUDIO.playSound('sfx_win');
                VFX.trigger('implode', boss.position, null, 0xffd700);
                
                // Loot
                const lootRoll = Math.floor(Math.random() * 6) + 1;
                addLog(`Boss Drop Roll: ${lootRoll} items found!`, "log-gold");
                
                for(let i=0; i<lootRoll; i++) {
                    const card = createItemInstance(DECK_TREASURE[Math.floor(Math.random() * DECK_TREASURE.length)]);
                    p.inventory.push(card);
                }
                
                // --- CLEANUP SPECIFIC DUNGEON ---
                p.inDungeon = false;
                p.dungeonProgress = 0;
                const typeToRemove = p.dungeonType; // Save type before clearing player
                p.dungeonType = null;
                
                // Find entrance tile to return to
                const entranceTile = tiles.find(t => t.userData.info.name === typeToRemove) || tiles[0];
                
                animateTeleport(p, entranceTile, () => {
                    // Remove Visuals for THIS dungeon only
                    if (activeDungeons[typeToRemove]) {
                        dungeonGroup.remove(activeDungeons[typeToRemove].group);
                        // Optional: Dispose of geometries to free memory here
                        delete activeDungeons[typeToRemove];
                    }
                    
                    // Sync Removal to DB (Host Only)
                    if (isMultiplayer && myPlayerId === 0) {
                        // Use the specific key to remove just this dungeon
                        // Note: Ensure your setupListener saves dungeons with keys if using this
                        // Fallback: Remove from the active list logic
                         db.ref(`games/${gameId}/dungeonActive/${typeToRemove}`).remove();
                    }
                ...
                if (p.bossAttempts > 0) {
                    addLog(`Boss blocked the attack! (${p.bossAttempts} tries left)`, "log-fail");
                    // Boss stays visible if End Boss. If Ambush, flavor choice:
                    if (isAmbush) {
                        addLog("The boss vanishes into the shadows...", "log-rare");
                        boss.visible = false;
                    }
                    // Update UI text for re-try
                    document.getElementById('enc-desc').innerText = `Attempts Left: ${p.bossAttempts}. Roll Highest Stat vs TN ${tn}.`;
                } else {
                    addLog(`${p.name} exhausted and fled the dungeon!`, "log-fail");
                    p.inDungeon = false;
                    p.dungeonType = null;
                    p.dungeonProgress = 0;
                    p.pos = 0; // Return to Inn
...
const NOTIFICATIONS = {
    ...
    updateBadge: function() {
        ...
        db.ref(`users/${currentUser.uid}/inbox`).once('value').then(snap => {
            ...
            // We store the last read timestamp in LocalStorage
    ...
    openModal: function() {
        ...
        const renderSystemMsg = () => {
            if (this.systemMsg && this.systemMsg.text) {
                ...
                div.innerHTML = `
                    <div class="system-update-badge">DEV UPDATE</div>
                    <div class="system-update-title">
                        <span>LATEST NEWS</span>
                        <span style="font-size:0.7rem; color:#aaa; font-weight:normal;">${dateStr}</span>
                    </div>
                    <div class="system-update-text">${this.systemMsg.text}</div>
                `;
        ...
        db.ref(`users/${currentUser.uid}/inbox`).once('value').then(snap => {
            ...
            snap.forEach(child => {
                ...
                if (n.type === 'dm') {
                    div.innerHTML = `
                        <div class="notify-header">
                            ...
                            <span style="cursor:pointer; color:#ef4444;" onclick="NOTIFICATIONS.delete('${child.key}')">×</span>
                        </div>
                ...
                // Append (so they appear below the system message)
                list.appendChild(div); 
            });
        });
    },
...
function init() {
    ...
    window.playCinematicIntro = (onComplete) => {
        ...
        // Assets Fetch
        const hKeys = Object.keys(CHAR_PORTRAITS);
        const heroes = [];
        for(let i=0; i<4; i++) {
            heroes.push(CHAR_PORTRAITS[hKeys[Math.floor(Math.random()*hKeys.length)]]);
        }
        ...
        heroesCon.innerHTML = heroes.map(url => `<img src="${url}" style="width:150px; height:200px; object-fit:cover; border-radius:10px; border:3px solid #fbbf24; background:#222;">`).join('');

        // T+0.5s Show BG
        timeouts.push(setTimeout(() => { bg.style.opacity = '0.5'; bg.style.transform = 'scale(1.0)'; }, 500));
        
        // T+3s Slide in Heroes
        timeouts.push(setTimeout(() => { heroesCon.style.left = '5%'; }, 3000));

        const showMonster = (index, delay) => {
            timeouts.push(setTimeout(() => {
                enemyCon.innerHTML = `<img src="${monsters[index].img}" style="height:350px; object-fit:contain; filter:drop-shadow(0 0 10px #000);">`;
                enemyCon.style.right = '5%';
                // Hit effect and hide
                timeouts.push(setTimeout(() => {
                    flash.style.opacity = '1';
                    overlay.classList.add('cine-shake');
                    setTimeout(() => { flash.style.opacity = '0'; overlay.classList.remove('cine-shake'); }, 100);
                    setTimeout(() => { enemyCon.style.right = '-50%'; }, 1000);
                }, 8000));
            }, delay));
        };

        // Monster timings to stretch to 1 minute
        showMonster(0, 8000);  // Ends 16s + 1s clear
        showMonster(1, 23000); // Ends 31s
        showMonster(2, 38000); // Ends 46s

        // T+48s Final Boss
        timeouts.push(setTimeout(() => {
            enemyCon.innerHTML = `<img src="${boss.img}" style="height:500px; object-fit:contain; filter:drop-shadow(0 0 20px #ef4444);">`;
            enemyCon.style.right = '5%';
            ...
            timeouts.push(setTimeout(() => {
                ...
                overlay.classList.add('cine-shake');
                setTimeout(() => { vs.style.transform = 'translate(-50%, -50%) scale(1.0)'; overlay.classList.remove('cine-shake'); }, 500);
            }, 5000));
            
            // Finish at T+60s
            timeouts.push(setTimeout(() => { cleanup(); }, 12000));
        }, 48000));
    };
...
function triggerTileEvent(p) {
    ...
    if (activeDungeons && activeDungeons[i.name] && !p.inDungeon && !p.isAi && !isMultiplayer) {
        offerDungeonEntry(p, i.name);
        return;
    }
...
function openSkillPicker(slotIdx, currentTab = 'active') {
    ...
    EQUIP_ORDER.forEach(slot => {
        ...
        if (item && item.ability) {
            const libEntry = Object.values(ABILITY_LIBRARY).find(s => s.name === item.ability.name);
            allSkills.push({
                ...item.ability,
                id: item.uid,
                source: item.name,
...
function showEncounter(p, i, t, mode, preDeterminedCard = null) {
    ...
    if (mode === 'enemy') {
         ...
         if (p.activeSkillId === 'siege_breaker' && !p.classSkillDepleted) {
             opts.unshift({ txt: "★ Siege Breaker", act: () => { 
    ...
    // D. STANDARD CHOICES (Wild Encounter / Skirmish)
        ...
        if (p.passiveSkillId === 'pacifist') {
            opts.push({ txt: "★ Flee (Pacifist +30G)", act: () => {
        ...
        if(p && pData) {
            ...
            if(pData.isPolymorphed !== undefined) {
                if (p.isPolymorphed !== pData.isPolymorphed) {
                    if (pData.isPolymorphed) { if (!p.isPolymorphed) makeSheep(p); } 
                    else { p.isPolymorphed = true; cureSheep(p); }
...
function updateHUD() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (skill) {
            ...
            txt.innerText = isDepleted ? "RECHARGE" : skill.name;
            if (skill.img) el.style.backgroundImage = `url('${skill.img}')`;
...
function openCharDetail(id) {
        ...
        <div class="sheet-scroll-area">
            ...
            <div style="padding: 15px;">
                <div class="paper-doll-container">
                    ...
                    <div id="cd-slot-head" class="doll-placement doll-slot" style="top: 5%; left: 5%;">HEAD</div>
                    <div id="cd-slot-body" class="doll-placement doll-slot" style="top: 25%; left: 5%;">BODY</div>
                    <div id="cd-slot-main" class="doll-placement doll-slot" style="top: 45%; left: 5%;">MAIN</div>
                    <div id="cd-slot-feet" class="doll-placement doll-slot" style="top: 65%; left: 5%;">FEET</div>
                    <div id="cd-slot-back"   class="doll-placement doll-slot" style="top: 5%; right: 5%;">BACK</div>
                    <div id="cd-slot-amulet" class="doll-placement doll-slot" style="top: 25%; right: 5%;">AMULET</div>
                    <div id="cd-slot-ring_l" class="doll-placement doll-slot" style="top: 45%; right: 5%;">RING L</div>
                    <div id="cd-slot-ring_r" class="doll-placement doll-slot" style="top: 65%; right: 5%;">RING R</div>
                    <div id="cd-slot-hands" class="doll-placement doll-slot" style="bottom: 5%; left: 25%;">HANDS</div>
                    <div id="cd-slot-off"   class="doll-placement doll-slot" style="bottom: 5%; right: 25%;">OFF</div>
                </div>
                ...
                <div id="cd-inv-grid-new" class="inv-grid" style="margin-top:10px;"></div>
            </div>
        </div>
    `;
    ...
    EQUIP_ORDER.forEach(slot => {
        const item = p.equipment[slot];
        const slotEl = document.getElementById('cd-slot-' + slot);
        if (slotEl) {
            if (item) {
    ...
    const invGrid = document.getElementById('cd-inv-grid-new');
    if (invGrid) {
        invGrid.innerHTML = '';
        for (let i = 0; i < 12; i++) {
            let d = document.createElement('div');
...
function showTooltip(data, isSkillId = false) { 
    ...
    if (isSkillId) { 
        ...
        if(data.ability) html += `<div class="tt-abil" style="color:var(--gold-glow)">${data.ability.name}: ${data.ability.desc}</div>`; 
    } 
...
function useSkill(slotIndex) { 
    ...
    const skillFn = skill.isClass ? ABILITY_LIBRARY[skill.id].fn : skill.ref.ability.fn;
...
function recalcStats(p) {
    ...
    if(pasDef && pasDef.effect) pasDef.effect(p.stats);
