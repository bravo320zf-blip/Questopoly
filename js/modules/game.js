        ...
        for (let i = 0; i < MAX_SLOTS; i++) {
            ...
            if (skillKey) {
                ...
                if (skill && skill.dungeonFn) {
                    ...
                    slot.onclick = (e) => {
                        e.stopPropagation();
                        // Check if already selected (by class)
                        if (slot.classList.contains('selected-skill')) {
                            // Double Tap -> Execute
                            useBattleSkill(skillKey);
                            slot.classList.remove('selected-skill');
                            hideTooltip();
                        } else {
                            // First Tap -> Select & Show Info
                            // Deselect others
                            document.querySelectorAll('.dungeon-skill-slot').forEach(el => {
                                el.classList.remove('selected-skill');
                                el.style.borderColor = '#444'; // Reset border
                            });

                            slot.classList.add('selected-skill');
                            slot.style.borderColor = 'var(--gold-main)'; // Highlight
                            showTooltip(skill);
