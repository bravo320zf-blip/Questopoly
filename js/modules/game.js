...
function spawnDungeonScenery(config, dungeonTiles, group) {
    ...
    candidates.forEach(pos => {
        ...
        if (config.bossName === "Countess Crimson") {
            if (Math.random() > 0.6) {
                // Creepy Tree
    ...
    if (config.bossName === "Countess Crimson") {
        ...
        // Orient towards the dungeon path
        mansion.lookAt(lastTile.position);
...
function createBossMesh(bossType, color) {
    ...
    else if (bossType === "Countess Crimson") {
...
const VFX = {
    audioMap: {
        ...
        firewave_burst: 'https://static.wixstatic.com/mp3/b16479_2e33d6685faf47269d73af40f44ed61e.mp3' // Re-use shockwave sound
    },
...
const ABILITY_LIBRARY = {
    // ================= FIGHTER SKILLS =================
    power_strike: {
        name: "Power Strike", desc: "Gain +5 STR for this turn only. (Dungeon: 10 DMG)",
        fn: (p) => {
            VFX.trigger('burst_up', p.mesh.position, null, 0xff0000);
            p.tempBonuses.str += 5; // Use temp folder
            addLog("Power Strike: STR boosted!", "log-success");
            return true;
        },
        dungeonFn: (battle) => {
            // Power Strike: Deal 10 DMG (Targeted)
            const target = battle.opponents[battle.targetIndex];
            if (!target || target.dead) return false;

            target.hp -= 10;
            playBattleAnim('lunge-right', 'player');
            setTimeout(() => playBattleAnim('hit', 'enemy'), 200);

            logBattleAction(`Power Strike! Dealt 10 DMG to ${target.name}.`);
            showFloatingText("10", "damage-popup", false);
            return true;
        },
    ...
    intimidate: {
        name: "Intimidate",
        desc: "Force player to pay 50G. Recharges on Enemy Capture. (Dungeon: Stun Enemy)", // UPDATED
        fn: (p) => {
        ...
        dungeonFn: (battle) => {
            // Intimidate: Monster skips next turn
            battle.opponentSkipping = true;
            logBattleAction(`Intimidated! Enemies skip next turn.`);
    ...
    second_wind: {
        name: "Second Wind", desc: "Gain 150 Gold immediately. (Dungeon: Heal 10 HP)",
        fn: (p) => {
            VFX.trigger('spiral_up', p.mesh.position, null, 0x00ff00);
            p.gold += 150;
            addLog("Second Wind: +150G", "log-gold");
            recalcStats(p); // Ensure stats are recalculated after skill use
            return true;
        },
        dungeonFn: (battle) => {
            // Second Wind: Heal 10 HP
            const heal = 10;
    ...
    shield_bash: {
        name: "Shield Bash", desc: "Stun a random enemy (Skip Next Turn). (Dungeon: Block + 3 DMG)",
        fn: (p) => {
            const targets = players.filter(t => t.id !== p.id && !t.isDead);
            if (targets.length) {
                const t = targets[Math.floor(Math.random() * targets.length)];
                VFX.trigger('shockwave', t.mesh.position, null, 0xffffff);
                t.isSkipping = true;
                addLog(`Shield Bash stunned ${t.name} !`, "log-epic");
                if (isMultiplayer && db && gameId) db.ref(`games / ${gameId} /players/${t.id} `).update({ isSkipping: true });
                return true;
            } return false;
        },
        dungeonFn: (battle) => {
            // Shield Bash: Block next attack & Deal 3 DMG
            const target = battle.opponents[battle.targetIndex];
            if (!target || target.dead) return false;
            ...
            setTimeout(() => playBattleAnim('shake', 'enemy'), 200);

            logBattleAction(`Shield Bash! Blocking + 3 DMG to ${target.name}.`);
            showFloatingText("3", "damage-popup", false);
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_d1f5ad06cfd64c688e68edd726ff65ae~mv2.jpg"
    },
    rally: {
        name: "Rally", desc: "Gain +2 to all Stats for this turn. (Dungeon: +5 HP, +5 DMG)",
        fn: (p) => {
        ...
        dungeonFn: (battle) => {
            ...
            battle.pHp = Math.min(battle.pMaxHp, battle.pHp + 5);
            target.hp -= 5;
            playBattleAnim('buff', 'player');
            playBattleAnim('hit', 'enemy');

            logBattleAction(`Rally! Healed 5 HP & Dealt 5 DMG to ${target.name}.`);
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_362057d2967143caacaea96b6def582f~mv2.jpg"
    },
    siege_breaker: {
        name: "Siege Breaker", desc: "Enemy Land: Roll 1d6 to kill Guards/Downgrade/Destroy.",
    ...
    fireball: {
        name: "Fireball",
        desc: "Pay 100G: Burn property. Guards die first. Splash 100G Dmg. (Dungeon: 6 DMG + Burn)",
        fn: (p) => {
            ...
            if (ts.length) {
                ...
                setTimeout(() => {
                    ...
                    // --- APPLY CHANGES ---
                    if (isMultiplayer) {
                        // In Multiplayer, we just update the DB. 
                        // The 'board' listener will see this and call applyCaptureVisuals for everyone automatically.
                        ...
                        syncPlayerState(); // Sync the 100G loss
                    } else {
                        // In Single Player, we must trigger the visual update manually.
                        // We find the owner object (if any remains) to give to the visual builder
        ...
        dungeonFn: (battle) => {
            // Fireball: 8 DMG Target, 4 DMG Others
            playBattleAnim('fireball', 'all-enemies'); // Trigger unique fireball anim on all
    ...
    transmute_gold: {
        name: "Transmute", desc: "Pay 50G: Destroy item -> Draw Treasure. (Dungeon: 15 DMG, Cost 20G)",
        fn: (p) => {
            // 1. Check Gold
            if (p.gold < 50) {
            ...
            return true; // Return true to indicate skill used (though effect is deferred, usually fine for Active skills)
        },
        dungeonFn: (battle) => {
            // Transmute: Pay 20G -> Deal 15 DMG
            const target = battle.opponents[battle.targetIndex];
            if (!target || target.dead) return false;
            ...
            if (battle.player.gold >= 20) {
                ...
                target.hp -= 15;
                playBattleAnim('buff', 'player'); // Cast
                setTimeout(() => playBattleAnim('hit', 'enemy'), 300);

                logBattleAction(`Transmute! Turned Lead to Pain (15 DMG).`);
                showFloatingText("15", "damage-popup", false);
                updateHUD();
                return true;
            } else {
    ...
    blink: {
        name: "Blink", desc: "Move 1-6 spaces immediately. (Dungeon: Dodge 100%)",
        fn: (p) => {
        ...
        // Blink: Dodge next attack (100%)
        dungeonFn: (battle) => {
            // Blink: Dodge next attack (100%)
            battle.playerDodging = true;
            playBattleAnim('vanish', 'player');
            logBattleAction(`Blink! You fade from reality.`);
            showFloatingText("VANISH", "block-popup", "player");
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_ead02548bf264a2eb097464789b65b11~mv2.jpg"
    },
    time_warp: {
        name: "Time Warp", desc: "Roll 1d6. On 5-6, take another turn immediately. (Dungeon: Extra Turn)",
        fn: (p) => {
        ...
        dungeonFn: (battle) => {
            // Time Warp: Take another turn immediately
            battle.extraTurn = true;
            logBattleAction(`Time Warp! Take another action.`);
            return true;
        },
    ...
    polymorph: {
        name: "Polymorph",
        desc: "Turn enemy into a Sheep (1 Turn). (Dungeon: Weakens Enemy)",
        fn: (p) => {
            ...
            if (targets.length > 0) {
                ...
                // Save who we were looking at (usually the active player 'p')
                const previousCameraTarget = cameraTarget;

                // Set focus to the Victim
                cameraTarget = t.mesh;
                isZoomed = true;

                // Animate Camera Position closer to victim
                const zoomOffset = { x: 0, y: 15, z: 12 };
                new TWEEN.Tween(camera.position)
                    .to({
                        x: t.mesh.position.x + zoomOffset.x,
                        y: zoomOffset.y,
                        z: t.mesh.position.z + zoomOffset.z
                    }, 800)
                    .easing(TWEEN.Easing.Quadratic.Out)
                    .start();

                // 2. Play "Freaked Out" Animation (High Jump + Fast Spin)
                new TWEEN.Tween(t.mesh.position)
                    .to({ y: 3 }, 400) // Jump high
                    .yoyo(true)
                    .repeat(1)
                    .start();

                new TWEEN.Tween(t.mesh.rotation)
                    .to({ y: t.mesh.rotation.y + (Math.PI * 8) }, 800) // Spin very fast
                    .onComplete(() => {
                        // 3. Poof Effect
                        ...
                        // CRITICAL FIX: Update camera to look at the NEW sheep mesh
                        // (Since makeSheep replaces t.mesh, the old cameraTarget reference is dead)
                        cameraTarget = t.mesh;
                        ...
                        // 5. Return Camera to Player after a short delay
                        setTimeout(() => {
                            // Restore target to active player
                            cameraTarget = (p.mesh) ? p.mesh : previousCameraTarget;

                            // Animate camera back to standard view
                            const stdOffset = { x: 0, y: 22, z: 18 };
                            new TWEEN.Tween(camera.position)
                                .to({
                                    x: p.mesh.position.x + stdOffset.x,
                                    y: stdOffset.y,
                                    z: p.mesh.position.z + stdOffset.z
                                }, 1000)
                                .easing(TWEEN.Easing.Quadratic.Out)
                                .start();
                        }, 1500); // Wait 1.5s so we can admire the sheep
                    })
                    .start();
        ...
        dungeonFn: (battle) => {
            // Polymorph: Weakens monster (Halves Dmg?)
            battle.monsterWeakened = 2; // Lasts 2 turns
            logBattleAction(`Polymorph! Monster is a Sheep.`);
            showFloatingText("BAAAH", "block-popup", "monster");
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_fbec81995dd34d0ebeb4b837d2e26b8d~mv2.jpg"
    },

    // --- UPDATED: ARCANE EYE (Teleport Fix) ---
    arcane_eye: {
        name: "Arcane Eye", desc: "Teleport to the nearest Chest. (Dungeon: Reveal Map/Loot)",
        fn: (p) => {
        ...
        // ADDED DUNGEON FUNCTION TO MAKE IT APPEAR IN BATTLE
        dungeonFn: (battle) => {
            // Logic: Low damage but maybe ignores defense? Or just reveal?
            // For now, standard damage to make it usable.
            const p = battle.player;
            const dmg = 2 + (p.stats.int || 0);

            // Apply Damage
            battle.mHp -= dmg;
            logBattleAction(`Arcane Eye reveals weakness! Dealt ${dmg} DMG.`);
    ...
    pickpocket: {
        name: "Pickpocket",
        desc: "Steal 10G/prop from the land leader. Recharges every 10 moves. (Dungeon: Steal Gold)",
        fn: (p) => {
    ...
    sprint_act: {
        name: "Sprint",
        desc: "Move 3 spaces. Recharges every 10 moves. (Dungeon: +50% Evasion)",
        fn: (p) => {
            VFX.trigger('burst_up', p.mesh.position, null, 0x00ff00);

            // --- FIX: SEPARATE SP/MP LOGIC ---
            if (typeof isMultiplayer !== 'undefined' && isMultiplayer && gameId) {
                // In MP, push to moveHistory so everyone (including me) animates it in sync
                // We use d1=3, d2=0 to signify a 3-step move
                db.ref(`games/${gameId}/moveHistory`).push({
                    pid: p.id, d1: 3, d2: 0, finalSteps: 3, startPos: p.pos,
                    timestamp: firebase.database.ServerValue.TIMESTAMP
                });
            } else {
                // In SP, animate immediately
                animateMove(p, 3);
            }
            return true;
        },
        dungeonFn: (battle) => {
            // Sprint: Increase Evasion
            battle.playerEvasion = 0.5; // 50% dodge chance
            logBattleAction(`Sprint! Evasion increased.`);
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_2a97d38d75f14a7c96eb980178f1bdf6~mv2.jpg"
    },

    // --- UPDATED: SMOKE BOMB (Teleport Fix) ---
    smoke_bomb_act: {
        name: "Smoke Bomb", desc: "Teleport to random space (Escape). (Dungeon: Escape Battle)",
        fn: (p) => {
        ...
        dungeonFn: (battle) => {
            // Smoke Bomb: Escape Battle (Success)
            logBattleAction(`Smoke Bomb! Escaped combat.`);
            endBattle(true); // Treat as win (survival)
            return true;
        },
    ...
    sabotage: {
        name: "Sabotage",
        desc: "Choose a Tavern to downgrade (Level 2 -> 1). (Dungeon: 5 DMG + Stun)",
        fn: (p) => {
            ...
            targets.forEach(t => {
                ...
                b.innerHTML = `
                    <span>${propName} <small style="color:#aaa">(${ownerName})</small></span>
                    <span style="color:var(--gold-main)">${value}G</span>
                `;

                b.onclick = () => {
                    // 3. Execute Sabotage on Click
                    m.classList.remove('active');
            ...
            return false; // Return false so it doesn't auto-deplete immediately (handled in onclick)
        },
        dungeonFn: (battle) => {
            // Sabotage: 5 DMG + Stun
            battle.mHp -= 5;
            battle.monsterSkipping = true;
            logBattleAction(`Sabotage! 5 DMG + Stun.`);
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_0de936527a24420a856fad92eab0835f~mv2.jpg"
    },
    gamble_act: { name: "Gamble", desc: "Pay 50G. 50% chance to win 200G.", fn: (p) => { if (p.gold < 50) return false; p.gold -= 50; VFX.trigger('projectile_coin', p.mesh.position, p.mesh.position); if (Math.random() > 0.5) { p.gold += 200; addLog("Gamble won: +200G!", "log-gold"); } else { addLog("Gamble lost.", "log-fail"); } updateHUD(); return true; }, img: "https://static.wixstatic.com/media/b16479_613c6b72c1984c8d93398e7f00ae2ee0~mv2.jpeg" },

    // --- UPDATED: SHADOW STRIKE (Teleport Fix) ---
    shadow_strike: {
        name: "Shadow Strike", desc: "Teleport to target, Steal 100G. -1 Stats. (Dungeon: 12 DMG)",
        fn: (p) => {
            ...
            showPlayerPicker("Shadow Strike Target", enemies, (target) => {
                ...
                target.gold -= amt; p.gold += amt;

                // Fix: Debuff the TARGET, not the user
            ...
            return false; // Return false to wait for picker
        },
        dungeonFn: (battle) => {
            // Shadow Strike: 12 DMG
            battle.mHp -= 12;
            logBattleAction(`Shadow Strike! Critical Hit (12 DMG).`);
            showFloatingText("12", "damage-popup", "monster");
            return true;
        },
    ...
    // ================= CLERIC SKILLS =================
    heal_spell: {
        name: "Heal", desc: "Clear negative status (Jail/Stun). (Dungeon: Heal 12 HP)", fn: (p) => { VFX.trigger('spiral_up', p.mesh.position, null, 0x00ff00); if (p.isSkipping) { p.isSkipping = false; addLog("Healed status!", "log-success"); return true; } addLog("No status to heal.", "log-fail"); return false; }, img: "https://static.wixstatic.com/media/b16479_6fbb98ea84a9479b98dd0441fd7152b6~mv2.jpg",
        dungeonFn: (battle) => {
            const heal = 12;
            battle.pHp = Math.min(battle.pMaxHp, battle.pHp + heal);
    ...
    smite: {
        name: "Smite",
        desc: "Instant Capture (or Upgrade Camp to Tavern) on current tile. (Dungeon: 8 DMG, x2 vs Demons)",
        fn: (p) => {
            const t = tiles[p.pos];
        ...
        dungeonFn: (battle) => {
            // Smite: 8 DMG (x2 vs Demons)
            let dmg = 8;
            if (battle.monster.name.includes('Demon') || battle.monster.name.includes('Balrog') || battle.monster.name.includes('Vampire')) dmg = 16;
            battle.mHp -= dmg;
            logBattleAction(`Smite! Dealt ${dmg} DMG.`);
            showFloatingText(`${dmg}`, "damage-popup", "monster");
            return true;
        },
    ...
    bless: {
        name: "Bless", desc: "Gain +5 INT for this turn. (Dungeon: Heal 5 + Luck)",
        fn: (p) => {
            VFX.trigger('pillar', p.mesh.position, null, 0xffffff);
            p.tempBonuses.int += 5; // Use temp folder
            addLog("Blessed!", "log-success");
            return true;
        },
        dungeonFn: (battle) => {
            // Bless: +3 to all Rolls
            // Simplified: Heal + Buff
            battle.pHp = Math.min(battle.pMaxHp, battle.pHp + 5);
            battle.statsBuff = true;
            logBattleAction(`Bless! Healed 5 HP + Luck Up.`);
            return true;
        },
        img: "https://static.wixstatic.com/media/b16479_f3d43368e934422fbaa0f1d3c93a8e29~mv2.jpg"
    },
    divine_intervention: { name: "Intervention", desc: "Roll 1d6. On 6, Gain 500G. (Dungeon: Chance for massive DMG/Heal)", fn: (p) => { VFX.trigger('pillar', p.mesh.position, null, 0xffffff); if (Math.floor(Math.random() * 6) + 1 === 6) { VFX.trigger('pillar', p.mesh.position, null, 0xffd700); p.gold += 500; addLog("Divine Gift: +500G!", "log-epic"); updateHUD(); return true; } addLog("Prayers unanswered.", "log-fail"); return true; }, img: "https://static.wixstatic.com/media/b16479_17b9cb42fac041e6ab622b3a45d6b3f0~mv2.jpg" },

    sanctuary: {
        name: "Sanctuary",
        desc: "Place Shrine. If Shrine exists, Teleport to it. (Dungeon: Heal to Full, Long CD)",
        fn: (p) => {
            ...
            if (p.sanctuaryPos !== undefined && p.sanctuaryPos !== null) {
                ...
                if (isMultiplayer && db && gameId) {
                    ...
                    // Cleanup data immediately for MP
                ...
                updateHUD();
                return true; // Successfully used -> Triggers Cooldown
            }
            // --- 2. PLACE SHRINE (If none exists) ---
                ...
                // MP Sync: Save the sanctuary position
                if (isMultiplayer && db && gameId) {
                    db.ref(`games/${gameId}/players/${p.id}/sanctuaryPos`).set(p.sanctuaryPos);
                }

                return false; // FIX: Do NOT trigger cooldown on placement (return false)
            }
        },
        img: "https://static.wixstatic.com/media/b16479_f33f291487d64a468e9a05b3811a0a3d~mv2.jpg"
    },
    exorcism: {
        name: "Exorcism", desc: "Destroy random enemy equipment (or -20G). (Dungeon: Remove Enemy Buffs + Dmg)",
        fn: (p) => {
            const enemies = players.filter(x => x.id !== p.id && !x.isDead && !x.isEventEntity);
            if (enemies.length === 0) return false;
            showPlayerPicker("Exorcism Target", enemies, (target) => {
                ...
                // REMOVED: p.classSkillDepleted = true;
                updateHUD();
            });
            return true;
        },
...
function animate() {
    ...
    // We only auto-track if we are NOT in a cinematic sequence
    if (!isCinematic && isZoomed && cameraTarget) {
...
function setupBattleScene(dungeonType) {
    ...
    if (dungeonType === 'Demon Gate') {
        ...
        // Pillars
        const colGeo = new THREE.CylinderGeometry(0.4, 0.4, 6, 8);
        const colMat = new THREE.MeshStandardMaterial({ color: 0x331111 });

        for (let x = -4; x <= 4; x += 4) {
            const col = new THREE.Mesh(colGeo, colMat);
            col.position.set(x, 3, -3);
            battleScene.add(col);
        }
...
function startBattle(p, opponent, type = 'dungeon') {
    // 1. Initialize State
    const isBoss = type === 'dungeon' && opponent.isBoss;

    // 4. Determine HP (Generic)
    let pHp, pMax;
    if (p.dungeonHp) {
        pHp = p.dungeonHp;
        const con = (p.stats && p.stats.con) ? p.stats.con : 10;
        pMax = p.pMaxHp || (con * 2);
    } else {
        // Fallback: If 'con' is missing, use 20 as base
        const con = (p.stats && p.stats.con) ? p.stats.con : 10;
        pHp = con * 2;
        pMax = con * 2;
    }
    ...
    // Create individual opponent instances
    for (let i = 0; i < count; i++) {
        opponents.push({
    ...
    if (type === 'dungeon' && p.dungeonType) envType = p.dungeonType;
    // Add logic for biome-based backgrounds for non-dungeon battles later

    if (DUNGEON_DATA[envType]) {
        const dungeonConf = DUNGEON_DATA[envType];
        bg.style.backgroundImage = `url('${dungeonConf.bg}')`;
    }
    ...
    activeBattle.opponents.forEach((op, index) => {
        ...
        sprite.src = op.img;
        sprite.className = 'battle-monster-sprite';
        ...
        if (count === 1) {
            ...
            if (index === 0) { sprite.style.left = '30%'; sprite.style.top = '50%'; }
            if (index === 1) { sprite.style.left = '70%'; sprite.style.top = '50%'; }
            sprite.style.transform = 'translate(-50%, -50%)';
        }
        else if (count === 3) {
            // Triangle
            if (index === 0) { sprite.style.left = '50%'; sprite.style.top = '30%'; sprite.style.zIndex = 10; }
            if (index === 1) { sprite.style.left = '25%'; sprite.style.top = '60%'; sprite.style.zIndex = 20; }
            if (index === 2) { sprite.style.left = '75%'; sprite.style.top = '60%'; sprite.style.zIndex = 20; }
            sprite.style.transform = 'translate(-50%, -50%)';
        }
        else {
            ...
            sprite.style.left = `${xBase + jitter}%`;
            sprite.style.top = `${yBase}%`;
            sprite.style.transform = 'translate(-50%, -50%)';
        }

        sprite.onclick = () => setBattleTarget(index);
        monsterContainer.appendChild(sprite);
    });
    ...
    // --- AI START CHECK ---
    if (p.isAi) {
        setTimeout(() => performAiBattleTurn(), 1500);
    }

    // --- UPDATE BUTTONS WITH STATS ---
        ...
        for (let i = 0; i < MAX_SLOTS; i++) {
            ...
            if (skillKey) {
                ...
                if (skill && skill.dungeonFn) {
                    slot.id = `battle-skill-${skillKey}`;
                    slot.onclick = () => useBattleSkill(skillKey);
                    // Native Tooltip
                    slot.title = skill.desc;
                    // Custom Tooltip
                    slot.onmouseenter = () => showTooltip(skill);
...
function useBattleSkill(skillKey) {
    ...
    if (p.battleCooldowns && p.battleCooldowns[skillKey] > 0) {
        showFloatingText("COOLDOWN", "block-popup", false);
        return;
    }
    ...
    activeBattle.turnProcessing = true; // Lock input

    // VALIDATE TARGET
    let target = activeBattle.opponents[activeBattle.targetIndex];
    if (!target || target.dead) {
        // Auto-select first alive
        const aliveIdx = activeBattle.opponents.findIndex(o => !o.dead);
        if (aliveIdx !== -1) {
            setBattleTarget(aliveIdx);
            target = activeBattle.opponents[aliveIdx];
        } else {
            // All dead? Should have ended.
            activeBattle.turnProcessing = false;
            // Force end check
            if (activeBattle.opponents.every(o => o.dead)) endBattle(true);
            return;
        }
    }

    // Execute Skill
    const success = skill.dungeonFn(activeBattle);

    if (success) {
        playBattleAnim('lunge-right', 'player'); // Player attacks
        // SET COOLDOWN (Default 3 turns)
        if (!p.battleCooldowns) p.battleCooldowns = {};
        p.battleCooldowns[skillKey] = skill.cooldown || 3;
        ...
        if (target.hp <= 0 && !target.dead) {
            target.dead = true;
            target.hp = 0;
            addLog(`${target.name} was defeated!`, "log-success");

            // Update visuals
            const spriteContainer = document.getElementById('battle-enemy-sprite');
            if (spriteContainer.classList.contains('horde-container')) {
                const sprites = document.querySelectorAll('.horde-sprite');
                if (sprites[activeBattle.targetIndex]) sprites[activeBattle.targetIndex].style.opacity = '0.2';
            } else {
                spriteContainer.style.opacity = '0'; // Single sprite fade
            }

            // Auto-switch target to next alive
            const nextAlive = activeBattle.opponents.findIndex(o => !o.dead);
            if (nextAlive !== -1) {
                setTimeout(() => setBattleTarget(nextAlive), 500);
            }
        }
        ...
        // Check Victory (ALL DEAD)
        const allDead = activeBattle.opponents.every(o => o.dead);
...
function resolveOpponentTurn() {
    ...
    // 1. Check Stun/Skip (Global for now, or per enemy? Global simplification: skip turn entirely)
    if (activeBattle.opponentSkipping) {
        logBattleAction(`Enemies are stunned!`);
        activeBattle.opponentSkipping = false;
        showFloatingText("STUNNED", "block-popup", true);
        return;
    }

    // Loop through ALL ALIVE opponents
    activeBattle.opponents.forEach((op, idx) => {
        if (op.dead || activeBattle.player.dead) return;

        // Enemy Attack Anim
        // We can try to animate specific sprite if we know index?
        // Using playBattleAnim('lunge-left', 'enemy') targets selected, which might not be attacker.
        // But visually "The Horde" attacks is okay.
        // Ideally we pass index to playBattleAnim? 
        // For now, generic enemy sprite shake/lunge is OK or main target lunge.

        // Let's animate the ATTACKER.
        const sprite = document.querySelectorAll('.horde-sprite')[idx];
        if (sprite) {
            sprite.classList.add('anim-lunge-left');
            setTimeout(() => sprite.classList.remove('anim-lunge-left'), 300);
        } else {
            playBattleAnim('lunge-left', 'enemy');
        }

        // Delay each attack slightly for visual pacing?
        // For simplicity, summing damage or instant sequential logs

        const mVal = (op.stats && (op.stats.str || op.stats.dex)) || 1; // Simplify stat choice
        const pBestStat = Math.max(activeBattle.player.stats.str, activeBattle.player.stats.dex, activeBattle.player.stats.int);

        // Rolls
        let mHits = 0;
        for (let i = 0; i < mVal; i++) { if (Math.random() * 6 + 1 >= 5) mHits++; }
        let pHits = 0;
        for (let i = 0; i < pBestStat; i++) { if (Math.random() * 6 + 1 >= 5) pHits++; }

        if (mHits > pHits) {
            let dmg = mHits - pHits;
            if (activeBattle.monsterWeakened > 0) { dmg = Math.floor(dmg / 2); activeBattle.monsterWeakened--; }
            activeBattle.pHp -= dmg;
            logBattleAction(`${op.name} attacks! You take ${dmg} DMG.`);
            showFloatingText(`${dmg}`, "dmg-popup", false);
            playBattleAnim('shake', 'player'); // Player gets hit
        } else {
            // Block
        }
    });
    ...
    if (activeBattle.pHp <= 0) {
        ...
        // --- AI TURN CHECK ---
        if (activeBattle.player.isAi) {
            setTimeout(() => performAiBattleTurn(), 1000);
        }
    }
}

function performAiBattleTurn() {
    if (!activeBattle || !activeBattle.player.isAi || activeBattle.turnProcessing) return;

    const p = activeBattle.player;

    // 1. Collect Available Skills
    let skillSet = new Set();
    if (p.activeSkillId) skillSet.add(p.activeSkillId);
    if (p.activeSkillId2) skillSet.add(p.activeSkillId2);
    if (p.assignedSkills) {
    ...
    // 2. Filter Cooldowns
    const validSkills = Array.from(skillSet).filter(key => {
        const cd = (p.battleCooldowns && p.battleCooldowns[key]) || 0;
        return cd === 0 && ABILITY_LIBRARY[key] && ABILITY_LIBRARY[key].dungeonFn;
    });

    if (validSkills.length > 0) {
        // Pick Random
        const choice = validSkills[Math.floor(Math.random() * validSkills.length)];
        // addLog(`${p.name} uses ${ABILITY_LIBRARY[choice].name}!`);
        useBattleSkill(choice);
    } else {
        // No skills? Just wait/skip? Or struggle?
        // For now, let's force a "struggle" or generic attack if possible, 
        // but since useBattleSkill requires a key, we might need a fallback.
        // Actually, most classes have 1 basic skill with 0 cooldown hopefully.
        // If truly nothing, just skip to avoid hang.
        addLog(`${p.name} hesitates...`, "log-fail");
        setTimeout(() => {
            resolveOpponentTurn();
        }, 1000);
    }
...
function resolveBattleRound(pStat) {
    if (!activeBattle) return;

    // 1. Monster Choice
    const stats = ['str', 'dex', 'int'];
    const mStat = stats[Math.floor(Math.random() * 3)];

    // 2. Check BLOCK (Same Stat)
    if (pStat === mStat) {
        logBattleAction(`BLOCKED! Both used ${pStat.toUpperCase()}. No damage.`);
        showFloatingText("BLOCKED!", "block-popup");
        return;
    }

    // 3. Resolve Rolls (Dice Pool: Stat Value = Number of D6s. 5,6 = Success)
    // 3. Resolve Rolls (Dice Pool: Stat Value = Number of D6s. 5,6 = Success)
    // 3. Resolve Rolls (Dice Pool: Stat Value = Number of D6s. 5,6 = Success)
    let pVal = (activeBattle.player.stats && activeBattle.player.stats[pStat]) || 1;
    ...
    // Support both Monster flat stats and Player nested stats object
    let mVal = 1;
    if (target.stats) {
        mVal = target.stats[mStat] || 1;
    } else {
        mVal = target[mStat] || 1;
    }

    let pHits = 0;
    for (let i = 0; i < pVal; i++) { if (Math.random() * 6 + 1 >= 5) pHits++; }

    let mHits = 0;
    for (let i = 0; i < mVal; i++) { if (Math.random() * 6 + 1 >= 5) mHits++; }

    // 4. Damage Calc
    let resultText = `You: ${pHits} hits (${pStat.toUpperCase()}) vs Enemy: ${mHits} hits (${mStat.toUpperCase()}).`;

    // Add Visual Dice
    const pDiceHtml = getDiceVisuals(pVal, pHits);
    // Enemy Dice (approximate visual since we don't know exact rolls per se, but we know total hits)
    // Actually we simulated it.

    // Append to log? Or use logBattleAction HTML support?
    // logBattleAction supports text. Let's make it support HTML if we trust it.
    // Actually, let's just append the dice row to the PREVIOUS log entry?
    // Or just make logBattleAction accept HTML.
    ...
    if (pHits > mHits) {
        // Player Wins
        dmgDealt = pHits - mHits;
        target.hp -= dmgDealt;
        setTimeout(() => playBattleAnim('hit', 'enemy'), 200);
        resultText += ` You deal ${dmgDealt} DMG!`;
        showFloatingText(dmgDealt, "dmg-popup", true); // True = on enemy
    }
    else if (mHits > pHits) {
        // Enemy Wins (Counter?)
        // const dmg = mHits - pHits; 
        // Player takes 0 on their own turn usually, or small chip?
        // Let's keep it safe: No damage to player on player turn unless critical fail?
        // User said "Enemy parried" previously.
        playBattleAnim('shake', 'player'); // Player gets hit (parried)
        resultText += ` Enemy parried!`;
    }
    else {
        resultText += ` Clashed! No damage.`;
    }

    // Custom Log with Dice
    ...
    entry.innerHTML = `
        ...
        <div style="display:flex; align-items:center; gap:10px; opacity:0.8;">
            <div style="display:flex; align-items:center;">Hero: ${pDiceHtml}</div>
        </div>
    `;
    // We omit enemy dice row to save space/complexity, or add it?
    // Let's add it only if meaningful.
    logContainer.prepend(entry);

    updateBattleUI();

    // 5. Check End Condition (ZOMBIE FIX)
    // Check if ALL are dead
    const allDead = activeBattle.opponents.every(o => o.dead || o.hp <= 0);

    if (allDead) {
        setTimeout(() => endBattle(true), 1000);
    } else if (activeBattle.pHp <= 0) {
        endBattle(false);
    }
}
...
function updateBattleUI() {
    ...
    if (target && target.hp <= 0 && !target.dead) {
        ...
        if (sprites[activeBattle.targetIndex]) {
            sprites[activeBattle.targetIndex].style.opacity = '0.3';
...
function endBattle(victory) {
    ...
    btn.style.display = 'none'; // Disable input

    // CAPTURE PLAYER REFERENCE SAFELY
    const player = activeBattle ? activeBattle.player : null;
    // Capture HP logic before activeBattle is potentially cleared or changed
    ...
    if (victory && player) {
        ...
        // Reset ONLY on Boss Victory (below), otherwise just tick down
        ...
        setTimeout(() => {
            ...
            if (activeBattle.isBoss) {
                // BOSS VICTORY: Reset Cooldowns
                player.dungeonCooldowns = {};

                // Boss Loot: 1d6 Treasures
                ...
                // Show visual for the first item
                setTimeout(() => {
                    const showItem = lootedItems[0];
                    if (lootedItems.length > 1) {
                        addLog(`(You also found ${lootedItems.length - 1} other items!)`, "log-rare");
                    }
                ...
                setTimeout(() => {
                    ...
                    player.pos = 0;
                    player.dungeonHp = null; // Reset HP
                    player.mesh.position.copy(tiles[0].position);
                    addLog("The Dungeon collapses! Warping to Inn...", "log-rare");
                }, 4000);

            } else {
                // Normal Loot / Overworld Loot
                // Count kills (all opponents in activeBattle)
                const totalMonsters = activeBattle.opponents.length;
                let treasureDraws = 0;

                for (let i = 0; i < totalMonsters; i++) {
                    if (activeBattle.type === 'dungeon') {
                        treasureDraws++; // Always draw in dungeon
                    } else {
                        // Roll 1d6 per monster
                        const roll = Math.floor(Math.random() * 6) + 1;
                        if (roll >= 5) treasureDraws++;
                    }
                }
                ...
                if (treasureDraws > 0) {
                    ...
                    for (let t = 0; t < treasureDraws; t++) {
                        ...
                        // Show first item visual only
                        if (t === 0 && !player.isAi) {
                            setTimeout(() => {
                                showEncounter(player, newItem, null, 'loot');
                            }, 500);
                        } else if (player.isAi) {
                            addLog(`${player.name} looted ${newItem.name}.`, "log-rare");
                        }
                    }
                } else {
                    addLog("No treasures found from these monsters.", "log-neutral");
                }

                // SAVE HP FOR NEXT FIGHT
                player.dungeonHp = finalHp;
                addLog(`Victory! HP: ${finalHp}/${maxHp}`, "log-success");

                endStep();
            }
            activeBattle = null;
        }, 1000);
    } else if (player && activeBattle.type === 'dungeon') {
        logBattleAction("DEFEAT... You flee the dungeon.");

        // DEATH PENALTY (Dungeon Only)
        const lostGold = Math.floor(player.gold / 2);
        player.gold -= lostGold;

        // ITEM LOSS LOGIC
        const candidates = [];
        player.inventory.forEach((item, idx) => candidates.push({ type: 'inv', index: idx, name: item.name }));
        ['head', 'body', 'main', 'off', 'feet'].forEach(slot => {
            if (player.equipment && player.equipment[slot]) {
                candidates.push({ type: 'equip', slot: slot, name: player.equipment[slot].name });
            }
        });
        ...
        if (candidates.length > 0) {
            ...
            lostItemName = choice.name;
            if (choice.type === 'inv') {
                player.inventory.splice(choice.index, 1);
            } else {
                player.equipment[choice.slot] = null;
                recalcStats(player);
            }
        }
        ...
        setTimeout(() => {
            ...
            player.inDungeon = false;
            player.pos = 10;
            player.dungeonHp = null;
            player.dungeonCooldowns = {};
            player.mesh.position.copy(tiles[10].position);

            addLog("You passed out and woke up outside.", "log-fail");
            addLog(`DEATH PENALTY: Lost ${lostGold}G and ${lostItemName}.`, "log-fail");
            activeBattle = null;
            if (typeof isMultiplayer !== 'undefined' && isMultiplayer) {
        ...
        setTimeout(() => {
            document.getElementById('battle-modal').classList.remove('active');
            addLog(victory ? "Battle Won!" : "Battle Lost.", victory ? "log-success" : "log-fail");

            if (activeBattle.type === 'overworld') {
                if (victory) {
                    continueLandingLogic(player, tiles[player.pos]);
                } else {
                    // Overworld Defeat: Pay Fine (10% Gold)
                    const fine = Math.floor(player.gold * 0.1);
                    player.gold -= fine;
                    addLog(`You fled and dropped ${fine}G.`, "log-fail");
                    endStep();
                }
            }
...
function rollMove() {
    ...
    if (!isMultiplayer) {
        ...
        // POLYMORPH FIX: Force roll to 1 if sheep
        if (p.isPolymorphed) {
            d1 = 1;
            d2 = 0;
            addLog("Baa! Sheep movement is limited.", "log-fail");
        }
        // TAX COLLECTOR FIX: Limit movement if debuffed
        else if (p.taxDebuff) {
            d1 = Math.floor(Math.random() * 6) + 1; // 1d6
            d2 = 0;
        ...
        // BUG FIX: Removed manual p.passiveSkillId === 'shadow_step' check here.
        // It is now handled by the Stat Engine in finishRollLogic.
...
function animateMove(p, s) {
    ...
    if (p.pos === 0) {
        if (isMe || isMyAi) {
            p.gold += 200;
            addLog("Passed Inn (+200G)", "log-success");
...
function rollForBattle(p, tile) {
    ...
    if (d20 <= 10) {
        ...
        startBattle(p, opponent, 'overworld');
    } else {
        // Safe -> Continue
...
function continueLandingLogic(p, tile) {
                    ...
                    act: () => {
                        if (p.gold >= 100) {
                            ...
                            showModal("Divine Reward", "Choose a PERMANENT stat boost:", [
                                { txt: "+1 STR", act: () => { p.permBonuses.str++; recalcStats(p); addLog("Gained Permanent +1 STR"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } },
                                { txt: "+1 DEX", act: () => { p.permBonuses.dex++; recalcStats(p); addLog("Gained Permanent +1 DEX"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } },
                                { txt: "+1 INT", act: () => { p.permBonuses.int++; recalcStats(p); addLog("Gained Permanent +1 INT"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } }
                            ]);
                        } else {
    ...
    // --- DUNGEON ENTRANCE LOGIC ---
    // If Player lands on an Active Dungeon Entrance, ask to enter.
    const tileInfo = tile.userData.info || {};
    if (tileInfo.isDungeonEntrance && !p.isAi && !isMultiplayer) {
        // Note: For MP, we might need different handling or rely on sync. 
        // But for local player (isMe check usually required, but here !isMultiplayer covers SP).
        // If MP, we need 'isMe' check like below. 
        // Let's defer MP logic or implement it if 'isMe'.
        // Wait, resolveLanding runs for everyone?
        // 'resolveLanding' is called after animation finishes.
        // In SP, it runs for AI too.
    }

    // Better Logic:
    const isLocalPlayer = (!isMultiplayer) || (p.id === myPlayerId);

    if (tileInfo.isDungeonEntrance && dungeonTiles.length > 0) {
        // Only ask Human Players. AI auto-enters? Or AI ignores? 
        // User said: "It is suppose to ask the player... If they say yes..."
        // Implies AI might skip it or roll for it. For now, AI skips.

        if (isLocalPlayer && !p.isAi) {
            showModal("Dungeon Gate", `You stand before the ${config.bossName}'s Domain. Dare you enter?`, [
                {
                    txt: "Enter Dungeon",
                    act: () => {
                        addLog(`${p.name} enters the Dungeon!`, "log-epic");
                        // Teleport to Dungeon Start
                        const startTile = dungeonTiles[0];
                        p.pos = startTile.userData.id;

                        // Move Mesh
                        createTeleportEffect(p.mesh.position);
                        p.mesh.position.copy(startTile.position);

                        // Sync
                        if (isMultiplayer) {
                            db.ref(`games/${gameId}/players/${p.id}/pos`).set(p.pos);
                            syncPlayerState();
                        }

                        // Trigger arrival at Dungeon Start
                        resolveLanding(p);
                    }
                },
                {
                    txt: "Stay Back",
                    act: () => {
                        addLog(`${p.name} turned away from the Dungeon.`);
                        triggerTileEvent(p);
                    }
                }
            ]);
            return; // Halt logic until choice made
        }
    }

    // --- SINGLE PLAYER LOGIC ---
    if (!isMultiplayer) {
        if (p.isAi) { triggerTileEvent(p); return; }
        ...
        // Start Tile
        const i = tiles[p.pos].userData.info;
        if (i.type === 'start') {
...
function continueLandingLogic(p, tile) {
    // --- 2. MEDIC PASSIVE CHECK (FIXED FOR PERSISTENCE) ---
    if (p.passiveSkillId === 'medic' && !p.isAi) {
        const patient = players.find(x => x.id !== p.id && !x.isDead && x.pos === p.pos);
        if (patient) {
            showModal("Medic", `Give 100G to ${patient.name} to gain +1 Permanent Stat?`, [
    ...
    if (!isMultiplayer) {
        ...
        // A. Dynamic Merchant Event Check
        const merchantEvent = activeEvents.find(e => e.data.type === 'merchant' && e.playerRef && e.playerRef.pos === p.pos);
        if (merchantEvent) {
            let stock = [];
            if (merchantEvent.data.id === 'merch_alchemist') stock = DECK_TREASURE.filter(it => it.type === 'scroll' && !it.name.includes('Gold'));
...
function continueLandingLogic(p, tile) {
            ...
            showModal("Medic", `Give 100G to ${patient.name} to gain +1 Permanent Stat?`, [
                {
                    ...
                    act: () => {
                        if (p.gold >= 100) {
                            ...
                            // Initialize Perm Bonuses if missing
                            if (!p.permBonuses) p.permBonuses = { str: 0, dex: 0, int: 0 };

                            // Sync Gold Changes Immediately
                            if (isMultiplayer) {
                                syncPlayerState();
                                db.ref(`games/${gameId}/players/${patient.id}/gold`).set(patient.gold);
                            }
    ...
    if (!isMultiplayer) {
        ...
        const tInfo = tiles[p.pos].userData.info || {};

        // A. Dungeon Entry Check (Top Priority)
        if (tInfo.isDungeonEntrance) {
            offerDungeonEntry(p, tInfo.name, () => {
                // Declined - Use Standard Logic
                runStandardLanding(p, tiles[p.pos]);
            });
            return;
        }
...
// This contains the rest of the original resolveLanding logic
// This checks Medic, then falls through to Merchant/Inn/Trigger
function runStandardLanding(p, tile) {
    // --- 2. MEDIC PASSIVE CHECK ---
    if (p.passiveSkillId === 'medic' && !p.isAi) {
        const patient = players.find(x => x.id !== p.id && !x.isDead && x.pos === p.pos);
        if (patient) {
            showModal("Medic", `Give 100G to ${patient.name} to gain +1 Stat?`, [
                {
                    txt: "Heal (Give 100G)",
                    act: () => {
                        if (p.gold >= 100) {
                            ...
                            showModal("Divine Reward", "Choose a permanent stat boost:", [
                                { txt: "+1 STR", act: () => { p.stats.str++; addLog("Medic: Gained +1 STR"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } },
                                { txt: "+1 DEX", act: () => { p.stats.dex++; addLog("Medic: Gained +1 DEX"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } },
                                { txt: "+1 INT", act: () => { p.stats.int++; addLog("Medic: Gained +1 INT"); if (isMultiplayer) syncPlayerState(); triggerTileEvent(p); } }
                            ]);
                        } else {
                            addLog("Not enough gold to Heal.");
                            triggerTileEvent(p);
...
function continueLandingLogic(p, tile) {
    ...
    // DEBUG: Check dungeon entrance logic
    const tInfo = tiles[p.pos].userData.info;
    if (tInfo.isDungeonEntrance) {
        debugLog(`[DUNGEON CHECK] Tile: ${tInfo.name}, Active? ${!!activeDungeons[tInfo.name]}`);
    }

    // --- DUNGEON ENTRY CHECK ---
    if (!p.isAi && activeDungeons[tInfo.name] && tInfo.isDungeonEntrance) {
        offerDungeonEntry(p, tInfo.name, () => {
            // On Decline: Proceed to standard landing logic (Upgrade/Siege)
            runStandardLanding(p);
        });
        return;
    }
    ...
    if (!isMultiplayer) {
        ...
        if (merchantEvent) {
            ...
            // Fallback if stock is empty
            if (stock.length === 0) stock = DECK_TREASURE.filter(it => !it.name.includes('Gold'));

            const items = [];
            for (let k = 0; k < 3; k++) {
                if (stock.length > 0) items.push(createItemInstance(stock[Math.floor(Math.random() * stock.length)]));
            }
            openCustomShop(p, items, merchantEvent.data.name, () => endStep());
            AUDIO.playSound('sfx_gold');
            return;
...
function triggerTileEvent(p) {
    ...
    // --- NEW: DUNGEON ENTRANCE CHECK ---
    // --- NEW: DUNGEON ENTRANCE CHECK (Removed - Handled in resolveLanding) ---
    ...
    if (m) {
        ...
        const closeBtn = m.querySelector('.close-x'); // Might be null
        const nextAction = () => continueFromArrival();

        btn.onclick = nextAction;

        // --- FIX: CHECK CLOSE BUTTON ---
...
function openSkillPicker(slotIdx, currentTab = 'active') {
    ...
    container.innerHTML = `
        ...
        <div id="skill-picker-list" style="padding: 20px; overflow-y: auto; flex-grow: 1; max-height: 400px;"></div>
    `;

    const list = document.getElementById('skill-picker-list');
...
function updateHUD() {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        if (skill) {
            ...
            el.style.opacity = isDepleted ? "0.4" : "1";
            txt.innerText = isDepleted ? "RECHARGE" : displayName;
...
function startGame() {
    ...
    if (isMultiplayer && gameId && db) {
        ...
        for (let i = 1; i < 4; i++) {
            ...
            const aiCol = aiColors.shift(); // Use shift() instead of splice to pick the next color safely
