...
function useBattleSkill(skillKey) {
    ...
    // Only allowed on Player Attack Turn
    if (activeBattle.turnState !== 'ATTACK') {
        showFloatingText("NOT YOUR TURN", "block-popup", false);
        return;
    }
...
function resolveOpponentTurn() {
    ...
    activeBattle.opponents.forEach((op, idx) => {
        ...
        if (mHits > 0) {
            // New Formula: Defense cuts hits in half, preventing total immunity
            let blocked = Math.floor(pHits / 2);
            let dmg = mHits - blocked;

            // Chip Damage: Ensure at least 1 damage if monster landed a hit
            if (dmg <= 0) dmg = 1;
            ...
            // Miss (mHits = 0)
            logBattleAction(`${op.name} missed!`);
        }
    });
...
// --- NEW BATTLE SYSTEM LOGIC ---

function startTurnSequence() {
    if (!activeBattle || activeBattle.turnQueue.length === 0) return;

    // Safety Wrap Index
    if (activeBattle.turnIndex >= activeBattle.turnQueue.length) activeBattle.turnIndex = 0;

    const currentEntity = activeBattle.turnQueue[activeBattle.turnIndex];

    // Skip Dead
    if (currentEntity.dead || (currentEntity.hp <= 0 && !currentEntity.isPlayer)) {
    ...
    if (currentEntity.isPlayer) {
        ...
        if (currentEntity.isAi) {
            activeBattle.turnState = 'ATTACK';
            log.innerHTML = `<span style="color:#aaa">${currentEntity.name} is thinking...</span>`;
            toggleBattleControls(false);
            setTimeout(() => automateActiveAiTurn(), 1000);
            return;
        }

        activeBattle.turnState = 'ATTACK';
        log.innerHTML = `<span style="color:#4ade80">YOUR TURN!</span> Select a Stat to Attack.`;
        // Unlock Buttons
        toggleBattleControls(true);
    }
    // 2. Monster Turn
    else {
        activeBattle.turnState = 'DEFEND';

        // AI Defense Check
        if (activeBattle.player.isAi) {
            toggleBattleControls(false);
            log.innerHTML = `<span style="color:#ef4444">${currentEntity.name} attacks!</span> AI is defending...`;
            setTimeout(() => automateAiDefense(), 1000);
            return;
        }

        log.innerHTML = `<span style="color:#ef4444">${currentEntity.name}'s Turn!</span> Guess their attack to Block!`;
        // Unlock Buttons (for defense choice)
        toggleBattleControls(true);

        // Auto-Attack animation preview?
    ...
    if (victory && player) {
        ...
        setTimeout(() => {
            ...
            if (activeBattle.isBoss) {
                ...
                let treasureDraws = 0;
                if (activeBattle.type === 'dungeon') treasureDraws = 1;
                else {
...
function continueLandingLogic(p, tile) {
    ...
    if (!p.isAi && activeDungeons && activeDungeons[tInfo.name] && tInfo.isDungeonEntrance) {
...
function updateHealthBar() {
    ...
    if (!p || !bar) return;
    if (typeof p.hp === 'undefined') { p.hp = 20; p.maxHp = 20; }
...
window.watchAiBattles = true; // DEFAULT: TRUE
