...
function continueLandingLogic(p, tile) {
    ...
    // --- DUNGEON ENTRY CHECK (Restored) ---
    const tInfo = tiles[p.pos].userData.info || {};
    if (tInfo.isDungeonEntrance && !p.isAi) {
        // For Single Player, or Multiplayer (if it's ME)
        if (typeof isMultiplayer === 'undefined' || !isMultiplayer || (p.id === myPlayerId)) {
