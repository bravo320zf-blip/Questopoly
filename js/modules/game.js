...
const ABILITY_LIBRARY = {
    ...
    ancient_tax: {
        name: "Tax Collector",
        desc: "Cost 200G. Steal 10% Gold from ALL players.",
        fn: (p) => {
            if (p.gold < 200) return false;
            p.gold -= 200;
            ...
            p.gold += totalStolen;
            addLog(`Tax Collector: Stole ${totalStolen}G total!`, "log-ancient");
            if (isMultiplayer) syncPlayerState();
            updateHUD();
            return true;
        },
    ...
    ancient_meteor: {
        ...
        fn: (p) => {
            ...
            // Remove 2 items
            let removed = 0;
            const slots = EQUIPMENT_SLOTS.filter(s => t.equipment[s]);
            for (let i = 0; i < 2; i++) {
                if (slots.length > 0) {
