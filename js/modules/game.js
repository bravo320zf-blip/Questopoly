...
const ABILITY_LIBRARY = {
    ...
    power_strike: {
        ...
        dungeonFn: (battle) => {
            ...
            const baseDmg = 10;
            const finalDmg = applyManaSurge(baseDmg, battle.player);
            target.hp -= finalDmg;
            playBattleAnim('lunge-right', 'player');
            setTimeout(() => playBattleAnim('hit', 'enemy'), 200);

            logBattleAction(`Power Strike! Dealt ${finalDmg} DMG to ${target.name}.`);
            showFloatingText(finalDmg.toString(), "damage-popup", false);
            return true;
        },
    ...
    if (success) {
        ...
        if (target.hp <= 0 && !target.dead) {
            ...
            if (sprites[activeBattle.targetIndex]) {
                const sp = sprites[activeBattle.targetIndex];
                sp.style.opacity = '0.2';
                sp.style.filter = 'grayscale(100%)';
            }
